...
def get_frames(video_path): #function to extract images from the given video
...
def convert_screenshots_to_pptx(img2, path):
    ...
    for j in k:
        if len(j) == 12 and i >= 1:
            ...
            if x < m and i >= 2:
                txBox = slide.shapes.add_textbox(left=7500 * min(lst1), top=7000 * (sum(lst2) / len(lst2)), width=w,
                                                 height=h)
                tf = txBox.text_frame
                tf.text = mytext
                # time.sleep(1)
